...
router.post('/api/login', (req, res, next)=>{
  const myFunction = passport.authenticate('local', (err,theUser, failureDetails)=>{
    ...
    if (!theUser) {
      //'failureDetails' contains the error message from our  logic in LocalStrategy {{ message: 'Email is taken, friend.' }}
    ...
    req.login(theUser, (err)=>{
      ...
      res.status(200).json(req.user); //req.user ili theUser svejedno je
    });
  });
